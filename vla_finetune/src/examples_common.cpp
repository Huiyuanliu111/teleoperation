...
bool MotionGenerator::calculateDesiredValues(double t, Vector7d* delta_q_d) const {
  ...
  sign_delta_q << delta_q_.cwiseSign().cast<int>();         // cwiseSign: take zhe sign of each element. if positive = +1. // cast<int>: double to int 
  Vector7d t_d = t_2_sync_ - t_1_sync_;
  Vector7d delta_t_2_sync = t_f_sync_ - t_2_sync_;          // decelerate
  std::array<bool, 7> joint_motion_finished{};

  for (size_t i = 0; i < 7; i++) {
    if (std::abs(delta_q_[i]) < kDeltaQMotionFinished) {
      (*delta_q_d)[i] = 0;
      joint_motion_finished[i] = true;                          // Joints that do not need to move
    } else {
      if (t < t_1_sync_[i]) {
...
void MotionGenerator::calculateSynchronizedValues() {
  ...
  // calculate the time required for each joint and then the joint that required the longest time can be determined
  for (size_t i = 0; i < 7; i++) {
    if (std::abs(delta_q_[i]) > kDeltaQMotionFinished) {
  ...
  double max_t_f = t_f.maxCoeff();    // the joint that required the longest time    
